...
install_dependencies() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        if [[ "$PKG_MANAGER" == "apk" ]]; then
            ...
            if apk search python3.14 &>/dev/null && sudo apk add --no-cache python3.14 py3-pip nodejs npm dnsmasq curl bash 2>/dev/null; then
                echo "Using Python 3.14"
            elif apk search python3.13 &>/dev/null && sudo apk add --no-cache python3.13 py3-pip nodejs npm dnsmasq curl bash 2>/dev/null; then
                echo "Using Python 3.13 (3.14 not available)"
            elif apk search python3.12 &>/dev/null && sudo apk add --no-cache python3.12 py3-pip nodejs npm dnsmasq curl bash 2>/dev/null; then
                echo "Using Python 3.12 (3.14 not available)"
            else
            ...
            if apt-cache show python3.14 &>/dev/null && sudo apt-get install -y python3.14 python3.14-venv python3-pip nodejs npm dnsmasq curl 2>/dev/null; then
                echo "Using Python 3.14"
            elif apt-cache show python3.13 &>/dev/null && sudo apt-get install -y python3.13 python3.13-venv python3-pip nodejs npm dnsmasq curl 2>/dev/null; then
                echo "Using Python 3.13 (3.14 not available)"
            elif apt-cache show python3.12 &>/dev/null && sudo apt-get install -y python3.12 python3.12-venv python3-pip nodejs npm dnsmasq curl 2>/dev/null; then
                echo "Using Python 3.12 (3.14 not available)"
            elif apt-cache show python3.11 &>/dev/null && sudo apt-get install -y python3.11 python3.11-venv python3-pip nodejs npm dnsmasq curl 2>/dev/null; then
                echo "Using Python 3.11 (3.14 not available)"
            else
            ...
            sudo $PKG_MANAGER install -y python3 python3-pip nodejs npm dnsmasq curl
        fi
    fi
...
EOF
    ...
    if command -v systemctl &> /dev/null; then
        ...
        echo "   Start with: sudo systemctl start ktizo-backend"
    else
...
main() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        echo "   Or use systemd:"
        echo "   sudo systemctl start ktizo-backend"
    fi
    echo ""
